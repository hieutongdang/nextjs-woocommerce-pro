...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  return {
    title: category ? `${category.name} - Sản phẩm` : 'Không tìm thấy danh mục',
    description: category ? `Khám phá sản phẩm trong danh mục ${category.name}.` : '',
  };
}
...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps & { searchParams: { page?: string } }) {
  ...
  if (!category) {
    return <div className="container mx-auto px-4 py-8">Không tìm thấy danh mục.</div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <Pagination
        ...
        baseUrl={`/san-pham/${category.slug}`}
      />
    </div>
  );
} 